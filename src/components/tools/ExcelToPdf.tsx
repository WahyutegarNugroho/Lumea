...
import { ExternalLink, ShieldCheck, Zap, ArrowRight, Database } from 'lucide-react';
...
function ExcelToPdf({ lang = 'en' }: Props) {
  const t = useTranslations(lang);
  const externalLink = "https://www.ilovepdf.com/excel_to_pdf";
        ...
        <div className="space-y-6 flex flex-col justify-center">
          <div className="bg-zinc-50/50 dark:bg-zinc-950/50 border border-zinc-200 dark:border-zinc-800 rounded-3xl p-8 space-y-8">
            ...
            <div className="pt-8 border-t border-zinc-200 dark:border-zinc-800 flex gap-5">
              ...
              <div className="space-y-2">
                <h4 className="font-bold text-zinc-900 dark:text-zinc-50 text-lg">{t('ui.privacy_guaranteed')}</h4>
                <p className="text-zinc-500 dark:text-zinc-400 text-sm leading-relaxed">
                  {t('ui.excel_to_pdf_privacy_desc')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-zinc-900 rounded-3xl p-8 text-white relative overflow-hidden group cursor-pointer" onClick={() => window.open(externalLink, '_blank')}>
             ...
             <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-emerald-500/20 to-transparent rounded-full -mr-16 -mt-16 blur-2xl"></div>
          </div>
        </div>
      </div>
    </div>
...
export default withErrorBoundary(ExcelToPdf);
