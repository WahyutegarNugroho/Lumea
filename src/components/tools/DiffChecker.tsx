...
function DiffChecker({ lang = 'en' }: Props) {
  ...
  const computeDiff = (oldArr: string[], newArr: string[]) => {
    ...
    const matrix = new Int32Array((n + 1) * (m + 1));
    ...
    for (let i = 1; i <= n; i++) {
      for (let j = 1; j <= m; j++) {
        if (oldArr[i - 1] === newArr[j - 1]) {
          ...
          matrix[idx(i, j)] = Math.max(matrix[idx(i - 1, j)], matrix[idx(i, j - 1)]);
        }
      }
    }
    ...
    while (i > 0 || j > 0) {
      if (i > 0 && j > 0 && oldArr[i - 1] === newArr[j - 1]) {
        ...
        i--; j--;
      } else if (j > 0 && (i === 0 || matrix[idx(i, j - 1)] >= matrix[idx(i - 1, j)])) {
        result.unshift({ type: 'added', value: newArr[j - 1] });
        j--;
      } else if (i > 0 && (j === 0 || matrix[idx(i, j - 1)] < matrix[idx(i - 1, j)])) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      {!isCompared ? (
        ...
        <div className="bg-zinc-950 rounded-2xl border border-zinc-800 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
          <div className="flex items-center justify-between p-8 border-b border-zinc-900 bg-zinc-950/50 backdrop-blur-xl rounded-t-[2rem]">
            <h3 className="text-white font-bold font-outfit text-xl flex items-center gap-3">
              <div className="w-10 h-10 bg-zinc-900 rounded-2xl flex items-center justify-center border border-zinc-800">
                <Columns2 size={20} className="text-zinc-500 dark:text-zinc-400" />
              </div>
              {t('ui.comp_result')}
            </h3>
            <div className="flex gap-4">
              <button 
                ...
                className="px-6 py-2 bg-zinc-900 text-zinc-500 dark:text-zinc-400 hover:text-white rounded-xl text-xs font-bold border border-zinc-800 transition-all active:scale-95"
              >
                {t('ui.edit_input')}
              </button>
              <button 
          ...
          <div className="bg-zinc-950 p-6 font-mono text-[13px] leading-relaxed max-h-[70vh] overflow-y-auto custom-scrollbar relative">
            ...
            {diffLines.length === 0 ? (
              <div className="py-20 text-center text-zinc-600 dark:text-zinc-300 font-bold italic">
                {t('ui.no_diff')}
              </div>
            ) : (
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-3xl p-10 shadow-sm flex items-start gap-8">
        ...
        <div className="space-y-3">
          <h4 className="text-lg font-bold text-zinc-900 dark:text-zinc-50">{t('ui.zero_dep_engine')}</h4>
